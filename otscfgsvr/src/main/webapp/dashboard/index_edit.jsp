		...
		<div class="row">
			...
			<div class="r_box" style="margin-top:0px;">
				<select name="" id="indexTypeEdit" style="width:145px;height:25px;background-color:#EBEBE4;" disabled="disabled">
					<option value=0 >ElasticSearch</option>
		...
		<div class="row" id="indexEditHbaseColumns" style="margin-top:0px;overflow-y:auto;max-height:300px;display:none;">
			<div class="tablebg2" id="indexEditHbaseColumn1" style="width:430px;height:40px;margin-left:30px;margin-bottom:5px;">
				...
				<div class="r_box" style="width:145px;">
					<%--<select name="indexEditHbaseColumnName" id="indexEditHbaseColumnName1" style="width:145px;height:25px;" ></select>--%>
					<input name="indexEditHbaseColumnName" id="indexEditHbaseColumnName1" type="text"  class="textbox1" style="width:145px;height:25px;" >
				</div>
				<div class="r_box" style="margin-left:15px;width:80px;">
					<input name="indexEditHbaseColumnType" id="indexEditHbaseColumnType1" type="text"  class="textbox1" style="width:60px;height:25px;" placeholder="列类型">
				</div>
				<div class="r_box" style="width:60px;"><input name="" id="indexEditHbaseColumnMaxLength1" type="text" class="textbox1" style="width:60px;height:25px;" placeholder="最大长度"></div>&nbsp;&nbsp;
			</div>
...
//    var indexTypes = ["string", "boolean","int32", "int64", "float32", "float64", "binary", "location", "location_rpt"];
    var indexTypes = ["string","blob", "int8","int16","int32", "int64", "float32", "float64","binary"];
    ...
    function indexTypeTransform(type) {
		if(type=="Hbase"){
		    return 1;
		}
		else {
...
//        $("#indexTypeEdit").attr("value", indexEditInfoInit["index_type"]);
        indexTypeNum = indexTypeTransform(indexEditInfoInit["index_type"]);
        $("#indexTypeEdit").attr("value", indexTypeNum);
//        if (indexEditInfoInit["index_type"] == 1) {
        if (indexTypeNum == 1) {
            ...
            var column_name;
            var column_type;
            column_name = indexEditInfoInit["index_key"][i]["col_name"];
            $("#indexEditHbaseColumn1").find("#indexEditHbaseColumnName1").val(column_name);
            for (var ii = 0; ii < record_table_columns.length; ii++) {
                if(column_name == record_table_columns[ii]["col_name"]){
                    column_type = record_table_columns[ii]["col_type"];
                    break;
				}
            }
//            $("#indexEditHbaseColumn1").find("#indexEditHbaseColumnType").val(indexEditInfoInit["index_key"][i]["col_type"]);
            $("#indexEditHbaseColumn1").find("#indexEditHbaseColumnType1").val(column_type);
//            if (indexEditInfoInit["index_key"][i]["col_type"] == "string" || indexEditInfoInit["index_key"][i]["col_type"] == "binary") {
            if (column_type == "string" || column_type == "binary") {
                $("#indexEditHbaseColumn1").find("#indexEditHbaseColumnMaxLength1").val(indexEditInfoInit["index_key"][i]["col_maxLen"]);
            }
            else {
                $("#indexEditHbaseColumn1").find("#indexEditHbaseColumnMaxLength1").css("display", "none");
            }
            for(i=1; i<indexEditInfoInit["index_key"].length; i++){
                var columns_name;
                var columns_type;
                columns_name = indexEditInfoInit["index_key"][i]["col_name"];
...
//                text += '<select name="" id="indexHbaseColumnName' + indexEditColumnId + '" style="width:145px;height:25px;" >';
//                text += '<option value=0 >\""  columns_name  \""</option>';
//                text += '</select></div>';
                text += '<input name="" id="indexEditHbaseColumnName' + indexEditColumnId + '" style="width:145px;height:25px;" type="text"  class="textbox1">';
                text += '</div>';
                text += '<div class="r_box" style="margin-left:15px;width:80px;"><input name="" id="indexEditHbaseColumnType' + indexEditColumnId + '" type="text"  class="textbox1" style="width:60px;height:25px;" ></div>&nbsp;&nbsp;&nbsp;&nbsp;';
                text += '<div class="r_box" style="width:60px;"><input name="" id="indexEditHbaseColumnMaxLength' + indexEditColumnId + '" type="text" class="textbox1" style="width:60px;height:25px;" ></div>&nbsp;&nbsp;';
...
//                $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnName").val(indexEditInfoInit["index_key"][i]["col_name"]);
                $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnName"+ indexEditColumnId).val(columns_name);
                for (var ii = 0; ii < record_table_columns.length; ii++) {
                    if(columns_name == record_table_columns[ii]["col_name"]){
                        columns_type = record_table_columns[ii]["col_type"];
                        break;
                    }
                }
                $("#indexEditHbaseColumn"+ indexEditColumnId).find("#indexEditHbaseColumnType"+ indexEditColumnId).val(columns_type);
//                $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnType").val(indexEditInfoInit["index_key"][i]["col_type"]);
//                if (indexEditInfoInit["index_key"][i]["col_type"] == "string" || indexEditInfoInit["index_key"][i]["col_type"] == "binary") {
                if (columns_type == "string" || columns_type == "binary") {
                    $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnMaxLength"+ indexEditColumnId).css("display", "block");
                    $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnMaxLength"+ indexEditColumnId).val(indexEditInfoInit["index_key"][i]["col_maxLen"]);
                }
                else {
                    $("#indexEditHbaseColumn" + indexEditColumnId).find("#indexEditHbaseColumnMaxLength"+ indexEditColumnId).css("display", "none");
                }
                indexEditColumnId++;
            }
            ...
            var i = 0;
            $("#indexEditSolrColumn1").find("#indexEditSolrColumnName").val(indexEditInfoInit["index_key"][i]["col_name"]);
            $("#indexEditSolrColumn1").find("#indexEditSolrColumnType").val(indexEditInfoInit["index_key"][i]["col_type"]);
            for(i=1; i<indexEditInfoInit["index_key"].length; i++){
                var text = '<div class="tablebg2" id="indexEditSolrColumn' + indexEditColumnId + '" style="width:430px;height:40px;margin-left:30px;margin-bottom:5px;">';
                text += document.getElementById("indexEditSolrColumn1").innerHTML;
                text += '<div class="r_box" style="width:10px;padding-top:3px;"><a href="#"><span class="glyphicon glyphicon-minus" title="删除列" onclick="removeIndexEditSolrColumn(' + indexEditColumnId + ');"></span></a></div>';
                text += '</div>';
                $("#indexEditSolrColumns").find("#indexEditSolrLastColumn").before(text);
                $("#indexEditSolrColumn" + indexEditColumnId).find("#indexEditSolrColumnName").val(indexEditInfoInit["index_key"][i]["col_name"]);
                $("#indexEditSolrColumn" + indexEditColumnId).find("#indexEditSolrColumnType").val(indexEditInfoInit["index_key"][i]["col_type"]);
                indexEditColumnId++;
                $("input[type='text']").focus(function() {
                    $("#indexUpdateStatus").html("");
                });
